...
int  main (int argc, char** argv) {
	...
	int TLP_thres = 32768; // 65536 32768
	int K_thres = 128; // atoi(argv[2]);
	...
	for (int j=0; j<BATCH; ++j){
		...
		//printf("b-strategy = %d \n", b_strategy[j]);
		if (TLP >(0.5* TLP_thres) && M[j]>tile_size[t_strategy[j]][0] && Kj <=K_thres)
		{	b_strategy[j] = 2;
			Kj += 16;
		//	printf("batch strategy[%d]  = %d \n ",j, b_strategy[j]);
		}
	}

	//b_strategy[0] = 1; b_strategy[1]=1; b_strategy[2]=1; b_strategy[3]=1;
	int *dev_Ba;
